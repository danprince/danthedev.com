...
* [npm][npm] and [yarn][yarn]/[yarn v2][yarnv2]/[pnpm][pnpm] for managing dependencies.
* [webpack][webpack]/[rollup][rollup]/[parcel][parcel] for bundling code and assets into minimal distribution artifacts.
* [eslint][eslint] or [tslint][tslint] for linting.
* [uglify][uglify]/[terser][terser]/[babel-minify][babel-minify]/[closure compiler][closure-compiler] for minification.
...
* [postcss][postcss]/[sass][sass] to improve on the syntax of CSS
...
I want to share my setup for building _small projects_ that don't rely on hundreds of megabytes of development dependencies. There are still some rough edges, but removing my reliance on build tools has helped restore some of the joy that I first felt when starting out with web development many years ago.
...
This can be concerning for developers who envision waiting minutes for hundreds of tiny files to import sequentially, but this only tends to happen in small projects when you insist on putting all of your classses and functions into separate files.

I don't quite know how this became so popular, but if the reason is that you find it easier to navigate to code through the file system, then [get better editor tools for navigating your codebase!](https://code.visualstudio.com/docs/editor/editingevolved#_go-to-definition)

There's a simple rule I follow. Put stuff that is conceptually related into one file. If that file is getting uncomfortably big, then narrow the concept, split and repeat.

At the start of a project, nearly everything is conceptually related and `index.js` is enough. As ideas become concrete and you start to understand the subdomains of the problems you are solving, natural boundaries tend to emerge.
...
If you don't have Python, or you want a server that will automatically reload the page after you make a change (I always want this) then I recommend [live-server](https://github.com/tapio/live-server) or [Browsersync](https://browsersync.io/) (both good for different things). If you want a binary that doesn't require npm, then try [devd](https://github.com/cortesi/devd).

## Dependencies
What about times when you need to use other people's modules too? 

Traditionally, you would use some combination of [npm][npm] and [yarn][yarn] to download those modules (and their dependencies) from the npm registry. By default, those modules (and their dependencies) can [execute arbitrary code](https://blog.npmjs.org/post/141702881055/package-install-scripts-vulnerability) on your machine after installation, so there's a lot of trust involved when hitting install. Let's hope you didn't make a typo either!

If you use npm, then you should probably run `npm config set ignore-scripts true`.

You can bypass npm entirely by importing our third party modules directly from a CDN instead. I like to use [Skypack][skypack] and [UNPKG][unpkg].
...
If you only use the dependency in a single file, then importing it directly from the CDN is fine, but as soon as you need to use it in multiple files then duplicating the exact import url becomes error prone.
...
This time last year I was crossing Russia by train, with lots of free time and very little connectivity. Not a typical development environment maybe, but sometimes you don't have internet access and it's frustrating when that gets in the way of working.

I was building [a web app](https://github.com/danprince/siberia) with a single dependency, and I ended up vendoring it (which still works brilliantly as an alternative) during a window of 4G whilst stopped at a city along the way.

## Syntax
Historically, certain browsers struggled to keep up with the latest specifications for the web. Supporting a reasonable percentage of users required you to restrict yourself to using only a _safe_ subset of languages (e.g. ES3 or ES5). Eventually, source-to-source compilers like [Babel][babel] appeared, which rewrite modern syntax in terms of safer syntax that works in older browsers.
...
For example (at the time of writing) modules are supported by [92.66% of users globally](https://caniuse.com/es6-module), async functions are at [94.11%](https://caniuse.com/async-functions), classes are at [95.51%](https://caniuse.com/es6-class) and so on. If in doubt about a given feature, swing by [caniuse.com](https://caniuse.com/) and see for yourself.
...
But unless you've been hiding from the JavaScript ecosystem over the past 10 years, you'll have seen a lot of JSX around. It's a deservedly popular way to describe markup for component based frameworks because of the visual parity with the HTML that it will eventually generate.

You may also know that JSX is a syntax alternative for a less succinct way to express the same idea with regular code.

```js
import { h } from "your-favourite-ui-framework";
...
That's good news, because you can skip the syntax transform and write your components like this instead, which is what I usually do with Preact.
...
The other syntaxes you might come across would be type annotations (and a handful of other syntax extensions—including JSX) in [TypeScript][typescript] or [Flow][flow]. The next section will mostly focus on achieving type safety without an additional compilation step.
...
This problem doesn't exist when you use type annotations and a static type checker. Your editor will let you know exactly which properties are required, which are optional and the types of each one.

It's absolutely true that you don't _need_ a type checker for smaller projects, but once you've spent any significant time using them, going back to untyped JavaScript can feel a bit like doing a blindfolded lucky dip into a bucket of sharp objects. I'll be damned if I have to give up type safety for my small projects.

What if I told you that you don't need TypeScript's syntax to make use of its type checker?
...
TypeScript exposes a large subset of its functionality through [JSDoc annotations](https://www.typescriptlang.org/docs/handbook/jsdoc-supported-types.html) that you can use directly in regular JavaScript files that can be delivered to a browser (or server environment) without a compilation step.

Most TypeScript syntax can be converted over to JSDoc annotations, which usually are slightly more verbose than the original syntax, but still enable all the same features for type checking and refactoring (renaming symbols, finding implementations, etc). It's also worth mentioning that often you can skip the annotations and let TypeScript infer the types too.
...
If (like me) you haven't jumped over to one of these newfangled editors, it's less likely you'll have a copy of TypeScript ready to go, and will therefore need either a global or local installation of the `typescript` package (which one you choose will depend on how picky you need to be about versions). This means re-introducing npm, but in a fully optional way. Your code will continue to work fine with or without TypeScript installed.
...
It's also easy to forget how cool View Source still is. If you've been hacking on something fun, then it's a wonderful feature of the web that other programmers can learn something new by reading your code, without any reverse engineering.

## Styling
For those that prefer to write regular old CSS, you're in luck because you don't have to change much about your workflow.

If you're used to using a bundler and importing your assets directly from JS files, then you'll have to adapt slightly to importing your CSS files from your HTML or [other CSS files](https://developer.mozilla.org/en-US/docs/Web/CSS/@import).

Most other people fall within one of the following camps.

### PostCSS / SASS
These are languages that you will have to say goodbye to if you want a tool-free setup. For some people this will be a deal-breaker. All I can say is that CSS is probably better than you remember it being.
...
The future of tooling for web development is far from bleak and despondent though. In fact, there are plenty of tools that represent an exciting direction of travel for bigger projects and teams.

* [Rome][rome] promises to be a unified toolchain for the whole of the web. With so many source-to-source tools that rely on parsing/printing ASTs as their first and final step, it seems like a no-brainer to build a set of tools that work on the same AST. Also cool: no dependencies.
...
* [Vite][vite] is another module-first build tool from the team behind Vue. When I have a project that outgrows a tool-free setup, Vite is my first port of call. I don't actually use Vue much, but Vite works brilliantly regardless.
...
* [swc][swc] is a JavaScript compiler with a bundler, but written in Rust.
...
If you make something cool, people are more likely to want to play with it and contribute if they don't have to spend 5 minutes installing tools to get it running.
...
These days it's either tricky or impossible to get some of my old projects running again because the tools I used are now outdated. At the time, they usually suggested using a global installation, so I have no idea which versions I was using and I don't have enough motivation to find out either.
